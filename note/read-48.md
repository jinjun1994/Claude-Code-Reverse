...
回答：这一站统一拼装 process-based teammate 的启动命令、继承 flags 和继承 env。它处理的是“如何把 leader 当前启动条件有选择地传给新 teammate”，而不是简单复刻整个命令行。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果 spawn 参数各处手写，不同 backend 启动出来的 teammate 会带着不一致的模式和环境。最难查的问题往往就来自这些继承差异。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题是，子执行体应继承父执行体多少上下文。继承过少会失真，继承过多又会把不该传的状态一并带走。
