...
回答：这一站继续向下钻 mailbox 的通用通信本质，强调每个 agent 一个 inbox 文件，agentName 是寻址键，结构化协议消息与普通消息共用同一通道。它是 swarm 通信的通用邮箱协议与投递层。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果通信层没有统一信封和统一寻址，后面所有 permission、shutdown、plan approval 都会各走各路。上层会看似灵活，底层却越来越难维护。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题是，团队内通信更适合事件总线还是收件箱模型。这里选择 inbox，是因为它天然兼容持久化、轮询和恢复。
