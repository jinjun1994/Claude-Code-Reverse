...
回答：这一站真正要解决的，是很多“调用模型做分析再记结果”的 hook 模式为何值得抽成工厂。`shouldRun`、`buildMessages`、`parseResponse`、`logResult` 这些变化点一旦抽出来，post-sampling 分析类功能就能统一生长。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果每个分析型 hook 都手写一遍模型调用与结果处理，重复代码会很快盖过真正的业务差异。更麻烦的是，模型选择、确定性参数和 analytics 追踪也会逐步漂移。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题，是系统怎样把“常见的 LLM 使用套路”沉淀成稳定框架。这个 helper 说明，连分析型调用也值得有自己的模式语言。
