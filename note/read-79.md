...
回答：这一站真正要回答的是，HTTP hook 的 SSRF 防护为什么要下沉到 DNS lookup 层，而不是只做 URL 字符串校验。因为只有在解析阶段拦截私网、链路本地和元数据地址，才能堵住连接前窗口。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果只靠上层 URL 白名单或请求前检查，遇到 DNS 解析变化和地址映射时就会留下空隙。那样防的是“看起来像危险地址”的请求，而不是“真正连到了危险地址”的请求。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题，是网络安全边界应该放在哪一层最可靠。这个文件给出的答案是：尽量靠近真实 socket 连接点。
