...
回答：这一站关心的是，权限规则字符串为什么必须有一套正式语法，而不是靠临时 split。它把 `Tool(content)` 这种 DSL 的解析、转义和历史别名归一化都收进同一边界。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果规则语言没有专门 codec，最先出问题的就是括号、反斜杠和 legacy 名称兼容。规则看似能写，实际却无法稳定 round-trip，也就无法安全持久化。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题，是配置语言怎样在长期演进中既能表达复杂性又能维持兼容。Parser 的价值，正在于把“字符串”变成可信协议。
