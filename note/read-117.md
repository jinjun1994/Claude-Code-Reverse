...
回答：这一站的重点是把“给用户看的回复”从普通模型文本中剥离出来，变成唯一可靠可见的输出通道。双特性门、opt-in 与附件 best-effort 上传，都围绕一个事实展开：回复必须稳定落到用户真正能看到的地方。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果继续允许模型把主要答案写在普通文本里，再在工具里只说一句 done，用户体验就会彻底断裂。系统会误以为自己答完了，而用户真正看到的却只是空壳提示。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题是，AI 的内部输出和用户可见输出能否被严格区分。BriefTool 的价值，就在于它把“说给谁听”变成系统级约束，而不靠模型自觉。
