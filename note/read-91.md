...
回答：这一站真正要解释的是，fork 子代理为什么不是简单复制一份 agent 定义。它还要处理递归 fork 守护、prompt 缓存一致性以及那份专门的 `FORK_AGENT` 协议。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果 fork 只是随手构造新 agent，最先出问题的就是无限递归和上下文漂移。agent 看似被复用，实际却可能不断放大自身结构风险。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题，是代理系统怎样在允许自我分工时仍守住层级边界。fork 能力的价值，取决于约束是否和能力一起出现。
