...
回答：这一站关心的是，agent 定义为什么需要一个专门加载中枢，而不是在用到时四处读目录。六类来源、frontmatter/JSON 两种格式、去重合并与缓存，决定了它本质上是一个 agent 注册系统。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果 agent 加载散在不同入口，来源优先级和去重规则就会不断被重复实现。那样新增一种来源时，整个 agent 生态都会被迫跟着改。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题，是平台怎样把“可定义代理”做成产品能力，而不是目录扫描技巧。加载中枢的意义，就在于把来源异构统一成能力清单。
