...
回答：这一站要解决的是 deferred tools 不在首轮工具池时，模型怎样还能找到它们。`select:` 精确选择、关键字评分、`tool_reference` 输出和描述缓存，说明它本质上是工具发现与按需加载的中间层。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果所有工具一开始都塞进 prompt，首轮上下文会迅速膨胀，缓存也更难稳定；若完全不暴露延迟工具，模型又永远用不到那些低频但重要的能力。两头都会伤。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：放大看，这一站在回答“系统能力越来越多时，如何不让可用性与上下文体积同步爆炸”。ToolSearchTool 给出的答案是把发现本身做成一项能力。
