...
回答：这一站关心的是，子代理真正运行时，为什么需要一台完整执行引擎，而不只是调用一次 query。它负责构建隔离态的 hooks、MCP、skills、权限和上下文，并把运行过程流式产出给上层。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果没有专门 runAgent 层，AgentTool 只能一边管生命周期一边管底层运行细节。这样会让 spawn、resume、async lifecycle 和产出流回传互相缠死。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题，是“运行一个 agent”到底意味着调用模型，还是搭起一个受控小环境。这个文件给出的答案显然是后者。
