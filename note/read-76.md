...
回答：这一站关心的是，prompt hook 为什么要被建模成一次受限的 LLM 条件判定，而不是随意的提示调用。它把模板展开、JSON 结果解析和 blocking/success 语义收进同一执行边界。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果 prompt hook 没有正式输出协议，模型返回的文本就无法稳定转成运行时结论。这样一来，所谓“用 LLM 验证条件”只会变成难以依赖的启发式。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题，是怎样让 LLM 参与自动化判断却不把系统交给随意文本。这里的答案是：让模型输出服务于协议，而不是取代协议。
