...
回答：这一站关心的是，异步 command hook 为什么不能只“启动后不管”，而必须有 registry 跟踪其生命周期。后台进程、stdout 轮询、最终 JSON 提取和清理通知，都要求一层集中管理。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果 async hook 只是 fire-and-forget，系统既不知道它何时完成，也不知道怎样把结果送回 UI 和模型。这样异步能力表面上有了，实际上没有可靠收尾。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题，是后台扩展任务怎样保持可见、可追踪、可回收。registry 的存在，说明异步不只是执行方式，更是一种需要被治理的运行态。
