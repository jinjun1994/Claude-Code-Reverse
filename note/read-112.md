...
回答：这一站把搜索交给服务端 Anthropic API 执行，客户端只负责发起、跟踪流式事件并格式化结果。重点不是搜索引擎本身，而是怎样把搜索变成一个可见进度、可控次数、要求带来源的工具调用。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果客户端自己乱接搜索 API，不同提供商、配额、结果格式和成本模型都会侵入本地执行层。那会让一个本该简单的搜索工具变成复杂的外部集成堆栈。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题是，实时外部知识如何被接入模型而不破坏响应秩序。WebSearchTool 选择把执行外包给服务端，其实是在重新划分“模型客户端”和“知识入口”的边界。
