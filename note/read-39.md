...
回答：这一站负责 in-process teammate 的启动注册和终止回收，明确区分 spawn 阶段只需最少宿主能力，而执行循环另有其处。它把身份、状态与 cleanup 一次性钉住。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果 spawn 时不统一建立这些对象，后面谁该更新 AppState、谁该负责 kill 和回收都会变得模糊。问题不会在开始暴露，而会在结束时集中爆发。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题是，复杂运行体的“出生协议”要不要足够正式。越是共享进程、共享资源，越需要出生时就把边界立清楚。
