...
回答：这一站关心的是，AgentTool 为什么需要一层足够复杂的专用 UI，而不是复用普通工具结果展示。子代理会产生多段进度、折叠组、摘要和多种终态，因此渲染本身就是一项正式工作。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果把 agent 执行过程硬塞进通用工具消息，用户只会看到噪声堆积而不是可理解进展。复杂度并不会消失，只会转成阅读负担。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题，是长过程工具怎样被呈现得既完整又可读。Agent UI 的价值，是把“执行历史”翻译成人能消费的进度叙事。
