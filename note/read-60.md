...
回答：这一站真正要解决的，是一次工具权限流里那些共享状态和共享操作为什么要被压成 `PermissionContext`。没有这层，日志、queue、abort、allow/deny 和 one-shot resolve 都会在各个 handler 里重复扩散。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果把这些共享能力散在不同 handler 里，每加一条审批路径都要重新接线一次。异步竞争一多，重复 resolve、状态清理不全之类问题就会集中爆发。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题，是异步编排系统如何给“一次流程”建立稳定的操作面。`PermissionContext` 的价值，是把流程身份和流程能力一起冻结下来。
