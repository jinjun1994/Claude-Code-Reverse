...
回答：这一站把代理间通信从普通文本发送扩展成一套正式消息协议，涵盖单播、广播、shutdown 和 plan 审批。五种路由路径并存，说明它真正承载的是 swarm 内部的组织通信，而不是聊天功能。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果所有消息都只是一段字符串，跨会话桥接、队友恢复、关闭握手和 leader 审批都会失去结构约束。系统最终会看起来能沟通，但没人知道哪条消息是在请求、哪条是在裁决。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题是，多智能体系统怎样拥有“内部政治”。SendMessageTool 的存在表明，一旦 agent 开始协作，通信协议就不再是附属，而是治理核心。
