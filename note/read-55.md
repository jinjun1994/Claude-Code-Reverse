...
回答：这一站的元问题，是 mailbox 定义好了之后，谁来真正消费它并把不同消息路由到正确运行时。它的职责不是存储消息，而是把 unread inbox 轮询变成 UI、状态和模型输入的运行时效果。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果没有统一 poller，各类结构化消息只能在调用点零散拉取和判断。这样一来，普通 teammate 文本与控制协议消息会混在一起，响应时机也无法稳定。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：背后更大的问题，是一个本地轮询型系统怎样模拟接近实时的协作控制面。`useInboxPoller` 证明，关键不在是否实时，而在是否有统一分发点。
