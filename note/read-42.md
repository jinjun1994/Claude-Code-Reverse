...
回答：这一站是 swarm 的 team metadata control plane，既管 team file 读写与成员状态维护，也管 session 结束时的目录、pane、worktree 清理。它把团队作为一个可持久化、可回收的实体来管理。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果只有当前身份辅助器而没有 team 实体管理，团队会像临时拼出来的运行态。恢复、清理和控制面变更都难有单一点。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题是，协作团队究竟是短暂上下文还是正式系统对象。这里显然把它当成后者，因此才有完整 metadata plane。
