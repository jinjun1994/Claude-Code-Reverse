...
回答：这一站真正要回答的是，HTTP hook 为什么首先是安全执行问题，而不是“发个 POST”问题。URL 白名单、header 环境变量插值、代理路由和 SSRF 防护共同定义了它的边界。

### 反

问题：**如果把这一站的设计反过来，会发生什么？**

回答：如果把 HTTP hook 当成普通网络请求封装，最先失守的就是内网访问和 header 注入这类问题。那样 hooks 扩展能力越强，反而越容易变成攻击面。

### 空

问题：**跳出当前文件名，这一站背后更大的问题是什么？**

回答：更大的问题，是可配置出站请求怎样在产品里被安全地开放出来。这个执行器说明，网络扩展要先回答安全，再回答功能。
